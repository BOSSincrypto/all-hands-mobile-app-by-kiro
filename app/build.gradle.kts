 ...
 * environment variables. When neither is present the release build falls back to
 * the debug identity so `assembleRelease` still succeeds for personal sideloading.
 */
...
android {
    namespace = "dev.openhands.mobile"
    compileSdk = libs.versions.compileSdk.get().toInt()

    defaultConfig {
        applicationId = "dev.openhands.mobile"
    ...
    lint {
        ...
        abortOnError = true
        // targetSdk 36 is the level Google Play requires as of 2026-08-31. compileSdk is
        // already 37 (needed by Compose 1.12), so this warning is expected, not a defect.
